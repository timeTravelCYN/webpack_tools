...
module.exports = {
    ...
    rules: [{
        ...
        */
        ...
        /* 
          ...
          这里先使用eslint-loader处理，返回的结果交给babel-loader处理，loader的处理顺序从最后一个到第一个，babel-loader用来编译js文件
        */
        use: ['babel-loader']
      },
...

      {
        ...
        test: /\.css$/,
        /* 
          先使用css-loader处理，返回的结果交给style-loader处理，cssloader将css内容存为js字符串，并且会把background，@font-face等引用的图片，字体文件交给指定的loader打包，类似上面的html-loader，用什么loader同样在loaders对象中定义
        */
...

      {
        /* 
          匹配各种格式的图片和字体文件
          上面html-loader会把html<img>标签的图片解析出来，文件名匹配到这里的test的正则表达式，css-loader引用的图片和字体同样会匹配到这里的test条件
        */
  ...
  /* 
    ...
    而plugin关注的不是文件的格式，而是在编译的各个阶段，会触发不同的事件，让你可以干预每个编译阶段
  */
  plugins: [
    /* 
      html-webpack-plugin用来打包入口html文件
      entry配置的入口是js文件，webpack以js文件为入口，遇到import，用配置的loader加载引入文件，但作为浏览器打开的入口html，是引用入口js的文件，它在整个编译过程的外面，所欲，我们需要html-webpack-plugin来打包作为入口的html文件
    */
