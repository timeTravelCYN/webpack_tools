...
module.exports = (options = {}) => {
  ...
  /* 
   当我们在命令行中执行: webpack --env.dev --env.server localhost
      ...
      rules: [{
...

        {
          ...
          */
          ...
          /* 
            hhtml-loader接受attrs参数，表示什么便签的什么属性需要调用webpack的loader进行打包
            比如<img>标签的src属性，webpack会把<img》引用的图片打包，然后src的属性值替换为打包后的路径
            使用什么loader代码，同样是在module.rules定义中使用匹配的规则
            如果html-loadre不指定attrs参数，默认值是img:src，意味着会默认打包《img>标签的图片，
